...
def run_train(P, logger):
    ...
    # === LLM-ENRICHED EMBEDDINGS (Contribution 1) ===
    ...
    if os.path.exists(enriched_path):
        ...
        print(f'[LLM] Loaded enriched embeddings: {text_weights.shape}')
    else:
    ...
    for epoch in range(0, P['num_epochs']):
        ...
        for phase in ['train', 'val']:
            ...
            # === DINOV2 LABEL PROPAGATION (Contribution 2) ===
            if epoch == 0 and phase == 'train' and os.path.exists(DINOV2_PROP_PATH):
                ...
                _ps = torch.stack(table_pseudolabel, dim=0).squeeze(1).cpu()
                ...
                _orig_dev = table_pseudolabel[0].device
                for _i in range(_N): table_pseudolabel[_i] = _prop[_i].unsqueeze(0).to(_orig_dev)
